...
class _AXK2ConcatQANorm(RMSNorm):
    ...
    def forward_cuda(self, x: torch.Tensor, residual=None, post_residual_addition=None):
        assert residual is None and post_residual_addition is None
        return self._cat(super().forward_cuda(x), x)

    def forward_native(
        self, x: torch.Tensor, residual=None, post_residual_addition=None
    ):
        assert residual is None and post_residual_addition is None
        return self._cat(super().forward_native(x), x)

    def forward_cpu(self, x: torch.Tensor, residual=None, post_residual_addition=None):
        assert residual is None and post_residual_addition is None
...
class _AXK2GatedOProj(RowParallelLinear):
    ...
    def forward(
        ...
        if input_.is_cuda:
            ...
            input_ = input_ * torch.sigmoid(
                gate.reshape(input_.shape).float()
            ).to(input_.dtype)
...
class AXK2ForCausalLM(DeepseekV2ForCausalLM):
    def __init__(self, config, quant_config=None, prefix: str = "") -> None:
        ...
        if self.dsa_enable_prefill_cp or self.mla_enable_prefill_cp:
            raise NotImplementedError(
                "Prefill context parallelism is not supported for A.X-K2 yet."
            )
